...
const PdfItem = ({ file, index, onRemove, onRotate }: PdfItemProps) => {
  ...
  return (
    <div className='group hover:border-brand-primary bg-brand-slate-50 relative flex h-[191.28px] w-[166.63px] items-center justify-center rounded-md px-[43px] py-[39px] hover:border'>
      ...
      {/* PDF Thumbnail */}
      <div className='transition-all' style={{ transform: `rotate(${file.rotation}deg)` }}>
        <div className='pointer-events-none relative flex items-center justify-center overflow-hidden select-none'>
          {['doc', 'docx'].includes(fileExtension ?? '') ? (
            ...
            <Image src='/images/pdf-tools/pdf-locked-icon.svg' alt='File locked icon' width={50} height={50} />
          )}
        </div>
      </div>
    </div>
