...
// Conversion options instead of compression levels
const CONVERSION_OPTIONS = [
  {
    id: 'pages',
    title: 'CONVERSION',
    description: 'Each page of the PDF is rendered and saved as an image.',
    value: 'pages',
  },
  {
    id: 'extract',
    title: 'EXTRACTION',
    description: 'The service extracts images that were embedded in the PDF file.',
    value: 'extract',
  },
];
...
// PDF to Images Options Component
...
const ConversionOptions = ({ selectedOption, onOptionChange }: ConversionOptionsProps) => {
  return (
    <div className='w-full'>
      <h3 className='mt-8 mb-4 text-lg font-semibold text-white'>PDF to Images options</h3>
              ...
              <div className='flex-1'>
                <div
                  className={`text-sm font-medium ${
                    selectedOption === option.value ? 'text-brand-primary-dark' : 'text-white'
                  }`}
                >
                  {option.title}
                </div>
...
const PreviewScreen = ({
  ...
  const [selectedConversionOption, setSelectedConversionOption] = useState<string>('pages');
    ...
    try {
      const response = await fetch('/api/ilovepdf/process', {
        ...
        body: JSON.stringify({
          ...
          tool,
          files: filePayload,
          pdfjpg_mode: selectedConversionOption,
        }),
      });
        ...
        >
          <div className='space-y-4'>
            ...
            <p className='text-brand-slate-600 font-inter rounded-[10px] bg-white p-[21px] text-base/[23px] font-normal'>
              Convert your PDF files to image formats. Choose from different conversion options based on your
              requirements.
            </p>
            ...
            <ConversionOptions
              ...
              onOptionChange={setSelectedConversionOption}
